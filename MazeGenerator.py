...
generator_types = {MAZE_GENERATOR_TYPE.RECURSIVE_BACKTRACKER:"Start",
					MAZE_GENERATOR_TYPE.RANDOM_PRIM:"Random Prim",
					MAZE_GENERATOR_TYPE.RECURSIVE_DIVISION:"Source and des",
					MAZE_GENERATOR_TYPE.UNION_FIND_SET:"A* search"}
...
def checkAdjacentPos(map, x, y, width, height, checklist):
	...
	if len(directions):
		...
		return False
# prime算法生成迷宫
# 算法详解：首先我们将点分为两个集合，一部分是可以作为通路的点，一部分为未添加到通路的点。随机选择一个起点，我将它标记为红色，判断它隔着蓝色格子对面
# 的格子是不是预定义的黄色，如果是，则将其标记为红色代表通路，将两个格子之间的蓝色格子也标记为红色代表通路。然后继续以最后标记的红色格子重复刚刚的操作。
